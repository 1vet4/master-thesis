...
optimize_funding <- function(df,
  ...
  if(!bootstrap) {
    ...
    y_pred <- a0 * df$EnrollmentPart + a1 * (F %*% w)
    ...
    # Predicted funding
    df$Predicted_Funding <- a0 * df$EnrollmentPart + a1 * (F %*% w_opt)
...
cv_lambda <- function(df, metric_names, lambda_values, a0, a1) {
  ...
  for (i in seq_along(lambda_values)) {
    ...
    for (yr in years) {
      ...
      test_pred <- test_df %>%
        mutate(Predicted = a0 * EnrollmentPart + a1 * rowSums(as.matrix(select(., all_of(metric_names))) * w))
  ...
  ggsave(
    filename = paste0("lambda_CV_a0_", a0*100, "_a1_", a1*100, "_baseline_model.png"),
    plot = cv_plot,
    width = 8,
    height = 4,
    dpi = 300
  ...
  print(weight_plot)
  ggsave(
    filename = paste0("weights_CV_a0_", a0*100, "_a1_", a1*100, "_baseline_model.png"),
    plot = weight_plot,
    width = 8,
    height = 4,
    dpi = 300
...
df_plot <- df %>%
  select(University_Code, Year, all_of(metric_names)) %>%
  pivot_longer(
...
ggplot(df_plot, aes(x = Year, y = Value)) +
  geom_line(linewidth = 1, color = "#2A5674") +
  geom_point(size = 2, color = "#2A5674") +
  facet_grid(
    Metric ~ University_Code,
    scales = "free_y",
    switch = "y"
  ) +
  scale_x_continuous(breaks = pretty(df_plot$Year, n = 4)) +
  labs(
    title = "Performance Metrics Across Universities",
    x = "Year",
    y = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
    ...
    panel.border = element_rect(fill = NA, color = "gray80", linewidth = 0.3)
  )
...
a0_85_a1_15_analysis <- analyze_funding_results(a0_85_a1_15_result$result_df,
                                                save_excel=TRUE,
                                                excel_data_path="../rezultatai/a0_85_a1_a15_results.xlsx",
                                                plot=TRUE,
                                                image_data_path="../images/results/a0_85_a1_a15_results.png")
...
a0_75_a1_25_analysis <- analyze_funding_results(a0_75_a1_25_result$result_df,
                                                save_excel=TRUE,
                                                excel_data_path="../rezultatai/a0_75_a1_a25_results.xlsx",
                                                plot=TRUE,
                                                image_data_path="../images/results/a0_75_a1_a25_results.png")
...
a0_5_a1_5_analysis <- analyze_funding_results(a0_5_a1_5_result$result_df,
                                              save_excel=TRUE,
                                              excel_data_path="../rezultatai/a0_5_a1_a5_results.xlsx",
                                              plot=TRUE,
                                              image_data_path="../images/results/a0_5_a1_a5_results.png")
...
a0_85_a1_15_result_jack_uni$w_sd
...
a0_75_a1_25_result_jack_uni$w_mean
a0_75_a1_25_result_jack_uni$w_ci
a0_75_a1_25_result_jack_uni$w_sd
